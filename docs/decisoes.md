...
## D16 — Experimentos como roteiros executados pelo próprio sistema
- **Contexto:** os experimentos do Capítulo 4 estavam em dois mundos. CT-02 a CT-05 rodavam contra a API do modo Demo (`docs/experimentos/funcional.py`), mas CT-07 (falsos positivos) e CT-09 (economia) eram programas em C++ à parte, que incluíam o `occupancy_fsm.h` e simulavam o PIR por conta própria. Os números do texto vinham desses programas, e não do sistema que a banca vê rodando. Faltavam três coisas para trazê-los para dentro: um jeito de descrever movimentação ("entra às 8h, move-se a cada 8 s, sai às 12h"), controle de tempo (uma jornada de 8 h levaria 8 h) e a ocupação de referência, sem a qual não há falso positivo nem linha de base.
- **Decisão:** roteiros declarativos em JSON (`docs/experimentos/roteiros/`, validados por zod no contrato) executados por `apps/backend/src/simulador/roteiro.ts` com relógio virtual, sobre o mesmo `passoDispositivo()` que o modo Demo usa em tempo real (`apps/backend/src/simulador/nucleo.ts`). `pnpm exp <arquivo-ou-pasta>` grava as leituras em CSV, as métricas em JSON e um `resumo.csv` por execução; `pnpm exp:latencia` mede o CT-06 no pipeline real, registrando se a medição passou pelo broker embutido ou externo. A variante de confirmação avaliada no Capítulo 4 virou o parâmetro `confirmacaoPorNivelMs` da FSM, nos dois lados (D14), em vez de existir só no programa do experimento.
- **Evidência:** os roteiros do CT-09 reproduzem a Tabela 3 (redução de 9,0% e 9,2% no perfil denso; 65,2% e 65,3% no esparso, com 7,25 h e 2,75 h de ocupação real). Os do CT-07 reproduzem a Tabela 2 em todas as 20 combinações de cenário e configuração — com uma diferença sistemática de um passo (0,1 s) explicada abaixo. Uma jornada de 8 h com passo de 500 ms roda em menos de 1 s.
- **Diferença de 0,1 s em relação aos programas em C++:** neles o tempo é um `double` acumulado (`t += 0.1`); depois de 150 somas, o instante nominal de 15 s vale 14,999999999999963. Com isso, `t - ultimoMovimento < 5,0` continua verdadeiro por mais um passo (a retenção do PIR vira 5,1 s) e `t >= 10` é falso no instante nominal de 10 s (a ocupação real começa um passo depois). O executor usa inteiros em milissegundos, onde `5000 < 5000` é falso. Onde o programa em C++ publica 35,1 s de acionamento indevido, o executor publica 35,0 s; onde publica 6,9 s de atraso, o executor publica 7,0 s. O padrão de ativação (quais configurações ativam em quais cenários) e o CT-09 inteiro são idênticos. **Ao atualizar o texto, usar os valores do executor**, que são os do sistema real.
- **Consequência:** cada número do Capítulo 4 é rastreável até um arquivo de roteiro versionado e reproduzível por um comando, sem Wokwi, sem hardware e sem programa avulso. Os testes `roteiros-versionados.test.ts` quebram se um roteiro deixar de produzir o valor publicado, então o texto e o código não se separam em silêncio. Os programas em C++ ficam no repositório como referência histórica da origem dos números. Limitação: o roteiro não mede latência de rede (não há rede), por isso o CT-06 continua sendo medido em tempo real pelo `exp:latencia`.
